...
**Decision:** Define application Zod schemas and migrations in `packages/database`, then configure the connection-owning `packages/firestore-database` facade. It validates persisted documents with strict Zod schemas on every read and write, owns all application transactions and migration write fencing, and prohibits runtime read-repair. Run ordered, forward-only data migrations in a dedicated Cloud Run Job before service deployment. Track migrations in Firestore with immutable IDs and checksums, resumable page checkpoints, and a transactionally acquired lease with fencing. Keep the generic facade free of application code for later extraction as an open source library.

**Rationale:** Firestore has no DDL schema, migration table, or collection-wide lock. A durable ledger provides the database-version behavior used by SQL migration tools, while checkpoints make large collection changes restartable and a fenced lease prevents concurrent runners. Repository write guards block mutations during migration and reject code whose registry differs from the database. Dedicated jobs avoid API startup timeouts; startup performs verification only. Forward-only expand/contract releases preserve compatibility while old Cloud Run revisions may still serve reads.
