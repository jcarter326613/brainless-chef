...
- A manual production dispatch requires `image_tag`: the full SHA of a development API and web release already deployed and tested. It promotes those images unless each production artifact is already retained for rollback.
...
The API service has no `allUsers` invoker binding. Callers need `run.routes.invoke`, normally through `roles/run.invoker`, and must send a Google-signed identity token.
